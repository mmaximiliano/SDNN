...
TEST_F(LollaTest, hackear_altera_mayor_gastador) {
    FachadaLollapatuza l(personas, puestos);
    l.registrarCompra(8, 4, 3, 2); //Compramos items sin descuento (hackeables).
    l.registrarCompra(2, 4, 3, 2);

    EXPECT_EQ(l.mayorGastador(), 2); //Como tienen mismo gasto, devuelve el de minimo ID.

    l.registrarCompra(8, 4, 2, 6);

    EXPECT_EQ(l.mayorGastador(), 8); //Como aumentamos gasto de 8, ahora se tiene que actualizar.

    l.hackear(8, 4);
    l.hackear(8, 4);
    l.hackear(8, 4);  //Hackeamos 3 veces, bajamos el gasto de 8.

    EXPECT_EQ(l.mayorGastador(), 2); //Debe volver a dar 2, vemos que se actualice el mayorGastador por Hackeo.
}
