...
TEST_F(LollaTest, arrancar_lolla) {
    ...
    EXPECT_EQ(l.personas(), personas); // se rompe aca.
...
TEST_F(LollaTest, hackear_cambia_max_gastador){
    //Crea una instancia de lolla
    set<Persona> personas = {1,2,3,4,5};
    const Menu menu = {{1, 300}, {2,500}, {3,100}};
    map<Nat, Nat> promo = {{3, 15}};
    const Promociones dtos = {{3, promo}};
    const Stock stocks = {{1,10}, {2,10}, {3,20}};
    Puesto p = Puesto(menu, dtos, stocks);
    map<IdPuesto, Puesto> puestos = {{1, p}};
    Lollapatuza lolla = Lollapatuza(puestos, personas);

    //La persona 1 compra 1 item de 500
    lolla.venta(2, 1, 1, 1);
    EXPECT_EQ(lolla.gastoDePersona(1),500);

    //La persona 2 compra 2 items de 300
    lolla.venta(1,2,1,2);
    EXPECT_EQ(lolla.gastoDePersona(2), 600);
    EXPECT_EQ(lolla.mayorGastador(),2);

    //La persona 2 compra 1 item de 100
    lolla.venta(3,2,1,1);
    EXPECT_EQ(lolla.gastoDePersona(2), 700);

    //Hackeamos un item de 300 a la persona 2
    lolla.hackear(1,2);
    EXPECT_EQ(lolla.gastoDePersona(2), 400);

    //Nos fijamos que el maximo gastador este bien actualizado
    EXPECT_EQ(lolla.mayorGastador(),1);
