...
fname = args.fname
final_destination = "/sequences/massive_runs/18/"
...
files = []
for nro in patt:
    files.append(os.listdir(os.path.join(root, str(nro))))
pattern_list = [[], []]

if fname[-1] == '8':
    final_destination += "p_18/"
    for i in range(0, 2):
        sample = random.choice(files[i])
        pattern_list[i].append(os.path.join(root, str(patt[i]), sample))

    train_seq = pan_functions.fixedPattern(root=root, nframes=nframes, wevents=wevents, patt=patt,
                                           nDigits=5000, pfreq=5, noiseNums=noiseNums, samples=pattern_list)
    test_seq = pan_functions.randomPattern(root=root, nframes=nframes, wevents=wevents, patt=patt,
                                           nDigits=2500, pfreq=10, noiseNums=noiseNums, samples=pattern_list)
elif fname[-1] == '5':
    final_destination += "p_18_5/"
    for i in range(0, 2):
        sample = random.choice(files[i])
        pattern_list[i].append(os.path.join(root, str(patt[i]), sample))
        sample = random.choice(files[i])
